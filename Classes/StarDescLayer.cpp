...
bool StarDescLayer::init(std::string mapid)
{
	...
	else
	{
		...
		int startx[] = { 180,120,50 };
		int offsetx[] = { 0,130,130 };
		int f3starawdsize = GlobalInstance::map_mapsdata[mainmapid].map_sublist[m_mapid].vec_f3starawds.size();
		for (int i = 0; i < 3; i++)
		{
			std::string resboxstr = StringUtils::format("resbox_%d", i);
			cocos2d::ui::ImageView* resbox = (cocos2d::ui::ImageView*)first3starboxnode->getChildByName(resboxstr);
			...
			if (i < f3starawdsize)
			{
				...
				std::string resstr = GlobalInstance::map_mapsdata[mainmapid].map_sublist[m_mapid].vec_f3starawds[i];
