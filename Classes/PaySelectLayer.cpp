...
bool PaySelectLayer::init(int payindex)
{
	...
	content->setString(GlobalInstance::vec_shopdata[payindex].name);

	cocos2d::ui::Text* price = (cocos2d::ui::Text*)csbnode->getChildByName("price");

	std::string pricestr =  StringUtils::format(ResourceLang::map_lang["rmbprice"].c_str(), GlobalInstance::vec_shopdata[payindex].price);
