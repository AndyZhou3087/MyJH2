...
SimpleRuleLayer::SimpleRuleLayer()
{
...
SimpleRuleLayer* SimpleRuleLayer::create(std::string ruletext)
{
	SimpleRuleLayer*pRet = new(std::nothrow)SimpleRuleLayer();
	if (pRet && pRet->init(ruletext))
	{
		pRet->autorelease();
		return pRet;
	}
...
bool SimpleRuleLayer::init(std::string ruletext)
{
	if (!Layer::init())
	{
		return false;
	...
	title->setString(ResourceLang::map_lang["simpleruletitle"]);
	...
	if (ruletext.length() > 0)
	{
		...
		ruleawardtext = CommonFuncs::replace_all(ruletext, "\\n", "\n");
